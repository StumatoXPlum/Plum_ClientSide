...
class Assets {
  static const String riveMoneyInAndOutPie = 'assets/rive/money_in_and_out_pie.riv';
}
...
class _InAndOutPieGraphState extends State<InAndOutPieGraph>
    ...
    for (int index = 0; index < items.length; index++) {
      ...
      if (index > 0) {
        ...
        animController.addListener(() {
          final value = pieValue * Curves.easeInOut.transform(animController.value);
          if (input != null) {
            input.value = value;
          }
        });
        ...
        pieAnimDelayTimers.add(
          Timer(Duration(milliseconds: riveValueIndex * delayBetweenPieSplitAnimations), () {
            if (mounted) {
  ...
  void onRiveInit(Artboard artboard) {
    final stateController = StateMachineController.fromArtboard(artboard, 'State Machine 1');
    if (stateController != null) {
      ...
      successTrigger = stateController.findInput<bool>("Success") as SMITrigger?;
      colorValueInput = stateController.findInput<double>("Color") as SMINumber?;
      ...
      valueNumber++) {
        final input = stateController.findInput<double>("Value0$valueNumber") as SMINumber?;
